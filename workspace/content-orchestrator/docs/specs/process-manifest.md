...
| `emart-revenue-health` | PM2 cron | `/root/emart-platform` | `bash /var/www/emart-platform/workspace/content-orchestrator/scripts/active/revenue_health_check.sh` | `*/30 * * * *` | server | [A] | cron/checker | no autorestart between cron runs | intentionally stopped | `/root/.openclaw/openclaw.env` | Tail PM2 out log if owner re-enables |
...
| `emart-meta-gen` | PM2 | `/root/emart-platform` | `bash /var/www/emart-platform/workspace/content-orchestrator/scripts/active/meta_gen_batch.sh` | on-demand | server | [A]/[X] | batch | PM2 autorestart historical | stopped; job complete | runtime env; no committed secrets | Keep stopped unless owner opens a reviewed meta batch |
