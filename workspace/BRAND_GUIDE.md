# Emart Brand SEO & Identity Guide

## Positioning

- **One-line:** "Korean-first. Global-complete. BD-delivered."
- Not a pure K-beauty store — global beauty hub with Korean leadership.

---

## Official Brand Names

Short name:

- Emart

Full name:

- Emart Skincare Bangladesh

Live domain / URL slug:

- e-mart.com.bd

Do not use:

- E-Mart BD
- EMart BD
- E-Mart
- Emart BD

## Brand Positioning

Emart Skincare Bangladesh is a locally verified health and beauty business serving Bangladesh with carefully curated authentic global beauty products.

Emart carries Korean beauty first, along with Japanese, UK, USA, France, Thailand, Chinese brand products, Taiwan, EU, aesthetic beauty items, derma cosmetics, beauty tools and color cosmetics.

## Brand Promise

Korean & Global Skincare Bangladesh.

Emart connects global-standard beauty products with local verification, customer support, faster delivery, and Bangladesh-focused shopping guidance.

## Trust Pillars

- Global standard
- Local trust
- Carefully curated products
- Authenticity verified
- Faster delivery
- Local verified business
- 8+ years in business
- Registered / verified business identity
- Dermatologist appointment support where available
- Creator, influencer and affiliate opportunity

## Business Theme

Emart is not just a product catalog. It should be presented as a trusted health and beauty platform for Bangladesh shoppers.

Core themes:

- Authentic Korean and global beauty
- Local verification and support
- Mobile-first Bangladesh shopping experience
- Concern-first skincare discovery
- Carefully curated product selection
- Creator, influencer and affiliate growth opportunity

## Approved Tagline

Korean & Global Skincare Bangladesh.

Currentness rule: if tagline or SEO wording sources disagree, live/search-facing output and `apps/web` metadata/source win over old decision notes, task logs, or session history. Exact-search old wording before and after edits, and ask the owner if the current wording is still unclear.

## Approved SEO Brand Usage

Homepage title:

```txt
Emart Skincare Bangladesh | Authentic Korean & Global Beauty
```

Homepage description:

```txt
Shop authentic Korean, Japanese and global skincare in Bangladesh from Emart Skincare Bangladesh. Carefully curated beauty products, local support, faster delivery and trusted service.
```

Product title format:

```txt
{Product Name} Price in Bangladesh | Emart
```

Brand page title format:

```txt
{Brand Name} Bangladesh | Authentic {Brand Name} Products | Emart
```

Category title format:

```txt
{Category Name} in Bangladesh | Emart Skincare Bangladesh
```

## Tone of Voice

Use a tone that is:

- trustworthy
- clean
- premium but friendly
- Bangladesh-aware
- skincare-guidance focused
- not overclaiming
- not fake medical/dermatology authority

Avoid exaggerated claims such as:

- cures acne
- removes pigmentation permanently
- dermatologist approved, unless verified
- guaranteed original without supporting process

Prefer safer wording:

- helps support
- suitable for
- carefully curated
- authenticity verified
- Bangladesh-friendly guidance
- local support
