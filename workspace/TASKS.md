...
| INFRA-4 | **`workspace/content-orchestrator/generated-assets/` (42MB, 339 files) has no `.gitignore` coverage** and keeps growing untracked-bloat-style, but unlike the social-calendar samples already fixed today, this looks like it may be a meaningful owner-review-gate audit trail (gated real-product model-shot pipeline, `project_model_shot_pipeline_20260629.md`) rather than disposable one-off junk. Owner decision needed: keep tracking as durable audit trail, or add a dated-subfolder ignore rule like the social-calendar fix. | [O] | 🔲 flagged 2026-07-01, awaiting owner call |
