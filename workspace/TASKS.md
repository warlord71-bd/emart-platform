...
- Internal SEO tool `--limit 20` sample rerun: Qdrant dim 768 and mpnet sidecar confirmed; outputs refreshed with 20 internal-link rows, 46 content-gap rows, 2 legitimate size-variant flags, 20 agentic-score rows. Four M.A.C NC shade false positives were reviewed by the owner and excluded; shade-code detection now prevents them from returning. OpenRouter generation timed out on primary+fallback, so deterministic fallback was used; owner review required before full-catalog run.
