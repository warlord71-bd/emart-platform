...
| INFRA-7 | **OpenClaw skill files (`/root/.openclaw/skills/`) had the same stale `/var/www/emart-platform/workspace/...` path bug as `ecosystem.config.cjs`/`process-manifest.md` — not previously audited since they live outside this repo.** Fixed `emart-competitor-prices`, `emart-humanizer`, `emart-meta-gen` `SKILL.md` (paths corrected to `/root/emart-platform/workspace/content-orchestrator/scripts/active/...`, verified against actual script output). **Two findings left unfixed, need owner decision:** (1) `emart-auto-publisher/SKILL.md` references `wp_auto_publisher.py`, which does not exist anywhere on the filesystem regardless of path — the skill is non-functional; likely superseded by OpenClaw's own `blog_generator.py` but not confirmed. (2) `emart-humanizer/SKILL.md` has a **plaintext DB password hardcoded** in its apply-mode bash instructions (`EMART_DB_PASSWORD="Emart@123456"`) — a real credential-exposure finding, not a path issue; should be rotated/moved to an env file the skill reads instead of embedding literally. | [O]+[C] | 🔲 paths fixed 2026-07-01; auto-publisher + password items awaiting owner call |
