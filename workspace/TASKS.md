# Emart Task Board
Last updated: 2026-06-26 (SEO source-of-truth reconciliation)
...
| 8 | Content / campaign safety | 8a WA-H, 8b SEO-ORCH-5, 8c UX-ORCH-6 | ✅ contracts done | WA-H blog draft gate done; content lifecycle contract at `workspace/docs/content-lifecycle-contract.md`; campaign orchestration contract at `workspace/docs/campaign-orchestration-contract.md`. |
...
| 8b | SEO-ORCH-5 | ✅ contract done | Content lifecycle contract at `workspace/docs/content-lifecycle-contract.md`: demand→brief→draft→QA→approve→publish→verify→measure→review. |
| 8c | UX-ORCH-6 | ✅ contract done | Campaign orchestration contract at `workspace/docs/campaign-orchestration-contract.md`: required fields, lifecycle, surface-specific rules, stale-content alerts. |
...
| 9b | UX-ORCH-5 | 🟡 spec done | Frontend health monitoring spec at `workspace/docs/frontend-health-monitoring-spec.md`. Implementation pending (provider choice: self-hosted JSONL recommended). |
| 9c | UX-ORCH-7 | 🟡 spec done | Experiment registry spec at `workspace/docs/experiment-registry-spec.md`. No active experiments; framework ready for post-freeze use. |
...
| 10a | ORCH-6 | 🟡 plan + integrity done | DR plan at `workspace/docs/disaster-recovery-plan.md`. Backup script hardened with fail-closed verification, checksum logging, and size anomaly detection. Off-server backup and restore drill require owner action. |
...
| 10e | SEO-ORCH-7 | 🟡 spec done | Off-page/entity/AEO ledger spec at `workspace/docs/off-page-entity-ledger-spec.md`. All outreach approval-first. |
| 10f | UX-ORCH-9 | 🟡 spec done | Customer feedback taxonomy spec at `workspace/docs/customer-feedback-taxonomy-spec.md`. Start with GA4 search no-result + 404 tracking. |
...
| D8 | Low | **AI Assistant is #2 BD channel** (334 sessions/14d) — `/brands/*` and `/best/*` pages drive most AI traffic. Original audit found 387 generic brand pages. Tier-1 implementation now uses static `brandEditorial.ts` for 15 brands with visible editorial copy + FAQ schema; 372 long-tail brands still use the generic fallback. Full audit at `workspace/audit/active/d8-brand-page-content-coverage-20260626.md`. | [C]+[O] | 🟡 Tier-1 live 2026-06-26; next is `USEO-9` Tier-2 brand editorial (15) after owner/copy review |
...
| SEO-4 | Expand buying guides for high-value categories. Tier-1 guides (korean-beauty, serums, toners-mists, bath-body, lips) are live; Tier-2 remains for body-wash, shampoos, and related categories. Face Cleansers/Sunscreen preserved. Full proposals at `workspace/audit/active/seo4-buying-guide-gap-proposals-20260626.md`. | [C]+[O] | 🟡 Tier-1 live 2026-06-26; Tier-2 tracked as `USEO-8` |
| SEO-5 | Add contextual links + FAQs to category guides. Existing face-cleansers/sunscreen and Tier-1 guide FAQ/contextual-link work is live; remaining work follows `USEO-8` Tier-2 category guides. All original link targets verified HTTP 200. Full proposals at `workspace/audit/active/seo5-contextual-link-faq-proposals-20260626.md`. | [C]+[O] | 🟡 Tier-1 live 2026-06-26; keep max 4-6 natural links per guide for Tier-2 |
...
| X4 — Social publishing: **2026-06-26** 24h random campaign is paused at owner review. First Simple face wash slot already published before review request; remaining 17 were regenerated in one unified Emart design (`random-24h-rescheduled`) with real logo, readable `E-MART.COM.BD` footer, updated no-wooden-base framework, 7-day duplicate guard, and La Roche 40ml replacement showing 2100. Live review contact sheet: `/images/social/2026-06-26/random-24h-rescheduled/contact-sheet.jpg`. Old PM2 schedulers `emart-fb-random-20260626` + `emart-ig-random-20260626` are stopped. 2026-06-25 Hydro Boost FB duplicate incident cleaned: 71 live matches reduced to 1 kept post (`106908734057777_1326993822890913`); `meta_schedule.js` now has result-ledger idempotency to prevent PM2 autorestart reposts. | 🟡 review ready; do not publish/reschedule until owner approves |
...
| SEO-ORCH-7 | Medium | Add governed off-page/entity/AEO operations: backlink and mention inventory, outreach/GBP/social-profile/review workflow with ownership and proof, toxic/spam review, AI citation/referral monitoring, and conversion attribution. Keep all external publishing/outreach approval-first. | 🟡 spec done 2026-06-26: `workspace/docs/off-page-entity-ledger-spec.md`; outreach/entity execution remains owner-gated. |
