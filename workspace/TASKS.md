...
| WA-I | 🟡 | **Stale `emart-meta-gen` PM2 path + orphaned legacy `workspace/scripts/` tree on VPS.** `ecosystem.config.cjs` registers `emart-meta-gen` (currently stopped, job complete 1,360/1,360) at `workspace/scripts/active/meta_gen_batch.sh` — the pre-consolidation path. Local/git only has `workspace/content-orchestrator/scripts/`; the VPS still carries a full untracked `workspace/scripts/` tree (~90 files, own active/archive/done split) never reconciled after the consolidation. Resuming the job today would silently run a stale VPS-only duplicate, not the current script. Found during 2026-06-30 docs reorg; not fixed. | [O]+[C] | 🔲 owner decision needed: repoint `ecosystem.config.cjs` to the current path, then audit/retire the VPS-only legacy tree |
