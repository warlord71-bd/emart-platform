...
| INFRA-4 | **`workspace/content-orchestrator/generated-assets/` bloat policy.** The immediate model-shot/owner-quality dirt is fixed in `31711bd`: generated model-shot request/metadata/holding outputs were removed from git tracking, current generated artifacts moved to `/root/.attic-2026-07-01/emart-generated-model-shot-residue/`, and future `generated-assets/model-shots/{external,holding,metadata,requests}` plus `generated-assets/owner-quality-*` outputs are ignored. Reusable source/reference assets (for example product cutouts) remain trackable. Broader generated-assets history/old blob cleanup is still covered by INFRA-6 and needs an owner maintenance-window decision. | [X]+[O] | 🟡 model-shot/owner-quality fixed 2026-07-01; broader history cleanup open |
| INFRA-5 | **Inconsistent attic/cleanup convention in use.** Documented standard (`.claude/rules/live-site-protection.md`) is `/root/.attic-YYYY-MM-DD/` (outside git, outside the repo). Codex's 2026-07-01 July-campaign cleanup instead used `workspace/.attic-2026-07-01/` (inside the repo, untracked hidden dir). Not broken, just inconsistent — a future agent checking "what got archived" could miss one convention. Needs a decision: pick one standard and note it, or explicitly allow both with clear rules for when each applies. | [O]+[C]+[X] | 🔲 flagged 2026-07-01, awaiting owner call |
| INFRA-6 | **`.git` repo is ~480MB, ~196MB of it tracked PNG/JPG history** (generated social/video campaign images committed directly across many past sessions). 2026-07-01 audit stopped new files of this kind from being tracked going forward (see INFRA-3-adjacent gitignore fixes, commit `810319e`), but this does NOT shrink existing history — the old blobs are still in every commit that touched them. Actually shrinking `.git` requires `git filter-repo` (or BFG) rewriting all commit hashes + a force-push to `origin/main`, which is disruptive (breaks any other clone/checkout, needs explicit sign-off — this is NOT something to run without direct owner approval). Decision needed: worth doing, and if so, schedule a maintenance window for it. | [O] | 🔲 flagged 2026-07-01, awaiting owner call |
| INFRA-7 | **OpenClaw skill files (`/root/.openclaw/skills/`) had the same stale `/var/www/emart-platform/workspace/...` path bug as `ecosystem.config.cjs`/`process-manifest.md` — not previously audited since they live outside this repo.** Fixed `emart-competitor-prices`, `emart-humanizer`, `emart-meta-gen` `SKILL.md` paths earlier. Closed the remaining findings 2026-07-01: confirmed `wp_auto_publisher.py` does not exist anywhere under `/root` and is superseded by OpenClaw's real `blog_generator.py`; updated `emart-auto-publisher/SKILL.md` to use `/root/.openclaw/workspace-emart/blog_generator.py --draft` and document the draft-gated topic-bank flow instead of the dead direct-publish CLI. Removed the plaintext `EMART_DB_PASSWORD` literal from `emart-humanizer/SKILL.md`; status/apply snippets now read `EMART_DB_PASSWORD` from the environment or `/root/emart-platform/apps/web/.env.local`, matching the existing `REVALIDATE_SECRET` sourcing pattern. Verified both skill files still have valid frontmatter and no `wp_auto_publisher.py` / plaintext DB password references remain. | [C] | ✅ 2026-07-01 |
...
| WA-I | 🟡 | **`emart-meta-gen` PM2 path fixed; broader VPS legacy `workspace/scripts/` reconciliation still open.** (1) ✅ **Meta-gen path fixed 2026-06-30:** `ecosystem.config.cjs` now points `emart-meta-gen` at `workspace/content-orchestrator/scripts/active/meta_gen_batch.sh` (job complete 1,360/1,360; confirmed the old VPS copy had stale `DOCS` pointing at `workspace/docs` which no longer exists). (2) 🔲 **Broader reconciliation open:** the VPS still has a full untracked `workspace/scripts/active/` legacy tree (~60 files) with multiple ecosystem/crontab entries pointing at it — including one LIVE process (`emart-social-fb-comments-20260629-v6 → meta_comment_queue.js`). Cannot archive that tree without first migrating those process entries to the `content-orchestrator/scripts/` equivalents. Crontab also has a live `checkout_monitor.js` entry at the legacy path. Needs owner decision + a full reconciliation pass before the legacy tree can be retired. | [C]+[O] | ✅ 2026-06-30 — meta-gen path fixed + all ecosystem/crontab legacy paths corrected |

### Blog Enhancement — ✅ fully closed, archived to `TASKS_ARCHIVE.md`
...
| VID-8 | **Persona/product-hand standard.** Sample pack created at `workspace/audit/archive/2026-06-30-active-cleanup/creative-usecase-samples-20260626/` covering blog OG → FB/IG → reel frames → local reel. Persona standard documented at `workspace/content-orchestrator/docs/claude-reference/creative-persona-standard.md`: product-first by default; best persona frame = exact real product in hand or beside face; model-only fallback is allowed only when Codex holding-shot generation is pending; white/tile product overlays are rejected as production standard. V2 product-card reels rebuilt 2026-06-26 with voiceover, caption overlays disabled, lower music, product cutout support, and Telegram review messages `25`/`26`. First exact-product holding-shot verification asset generated for Medicube PDRN from the real cutout and saved at `workspace/content-orchestrator/generated-assets/model-holding-product-medicube-pdrn-20260629.png` with owner-review gate metadata. Gated `model_shot.py` pipeline committed in `1c55ecf`: request/metadata/source cutout/fulfilled clean composite now live under `workspace/content-orchestrator/generated-assets/model-shots/`, video jobs fail closed if exact product image is missing. | [X] | 🟡 pipeline committed + verified 2026-06-29; owner visual approval still required before campaign use |
...
| CO-9 | **Standardize high-quality social/reel production lane.** Audit 2026-07-01 found the engines are healthy but the production contract was split: daily-producer reels had rich five-frame specs, voice-required QA, product-type templates, and master QA, while Content Orchestrator dispatch and Hermes queue actions could emit thinner drafts. Fixed with canonical `content_pack.py` + `docs/specs/content-pack-contract.md`: product source, exact image/cutout source, deterministic product-class script, voiceover/provider, frame list, platform sizes, safe-zone, QA gates, cost tier, owner approval state. Wired into Content Orchestrator dispatch, daily reel producer, and Hermes enqueue/reel pipeline; default tier remains free, escalation only on QA failure or owner approval. | [X]+[C] | ✅ 2026-07-01 in `dd18759` — verified py_compile, video quality-gate tests, Social Engine tests, Hermes import, daily-producer spec smoke, dry-run gated dispatch smoke; no publish/deploy/Woo writes. |
| CO-10 | **Architecture/logic-tree gap audit.** Rechecked `themes → resolver → plan → dispatch → content_pack/v1 → Social/Video/brief/model-shot → approval gate` and Hermes/OpenClaw-facing paths after CO-9. Fixed gate propagation (`owner`/`content` no longer collapse to `campaign`), static-only frame contracts, Hermes QA readback, remote product-image handoff, and generated Meta scheduler fallbacks away from retired `/var/www/.../workspace`. Audit note: `docs/audits/content-orchestrator-architecture-gap-20260701.md`. | [C] | ✅ 2026-07-01 in `dd18759` — verified py_compile, `node --check`, video quality-gate tests, Social Engine tests, in-memory content-pack smoke, and dry-run owner-gated dispatch; no publish/deploy/Woo writes. |
...
| 5 | ✅ Runtime/state map docs | ORCH-2, ORCH-4, ORCH-8 | ✅ Versioned process manifest + freshness/SLO spec created in `workspace/content-orchestrator/docs/specs/process-manifest.md`; durable queue/state contract created in `workspace/content-orchestrator/docs/specs/queue-state-contract.md`. | Docs/spec complete in `328572a`; implementation remains approval-first. |
...
### Fresh SEO Gap Action Queue — 2026-06-26 — ✅ fully closed, archived to `TASKS_ARCHIVE.md`
...
| P3-5 | USEO-11 | Continue humanizer for non-humanized PDP sections | RC-2 | SEO-7 F1 | existing engine, GSC order | 🟢 ongoing — 2026-07-01: flagged count 560. **Real remaining backlog ≪ "3,503":** of 2,868 unflagged (excl 198 holdout), only 10 THIN + 601 PARTIAL are weak; 2,257 already STRONG/GOLDEN (avg score 83). Stripping hair/makeup/accessory/perfume/body-wash/masks → **humanizer-fit skincare backlog = 139 PARTIAL, mostly 0-sales long-tail.** Priority lists: `workspace/humanizer/impression-priority/active/priority-queue-{,skincare-fit-,haircare-makeup-}20260701.json`. This session: batch19 (5 PDPs) live, 74259 GMC "prescription" hard-fail fixed, 183 organic passes flagged (`organic-pass-20260701`, reversible), residue_lint gate hardened (list + em-dash≤2.2/100w + 500-750w). Detached engine run launched. **Blocker: paid OpenRouter exhausted (30/30.2); free models 429-throttled → generation stalls. Fast path = small credit top-up, clears 139-fit in <1hr.** |
...
| 8 | Content / campaign safety | 8a WA-H, 8b SEO-ORCH-5, 8c UX-ORCH-6 | ✅ contracts done | WA-H blog draft gate done; content lifecycle contract at `workspace/content-orchestrator/docs/specs/content-lifecycle-contract.md`; campaign orchestration contract at `workspace/content-orchestrator/docs/specs/campaign-orchestration-contract.md`. |
...
| 5a | ORCH-2 | ✅ docs complete | Versioned PM2/cron/detached/worker manifest created in `workspace/content-orchestrator/docs/specs/process-manifest.md` (`328572a`). |
| 5b | ORCH-4 | ✅ spec complete | Freshness/SLO spec for last-success signals, duration, stale thresholds, retry/error classes, health checks, and recovery actions added to `workspace/content-orchestrator/docs/specs/process-manifest.md` (`328572a`). |
| 5c | ORCH-8 | ✅ contract complete | Shared durable queue/state contract for locks, atomic writes, idempotency, retries, dead letters, recovery, and audit trails created in `workspace/content-orchestrator/docs/specs/queue-state-contract.md` (`328572a`). |
...
| 8b | SEO-ORCH-5 | ✅ contract done | Content lifecycle contract at `workspace/content-orchestrator/docs/specs/content-lifecycle-contract.md`: demand→brief→draft→QA→approve→publish→verify→measure→review. |
| 8c | UX-ORCH-6 | ✅ contract done | Campaign orchestration contract at `workspace/content-orchestrator/docs/specs/campaign-orchestration-contract.md`: required fields, lifecycle, surface-specific rules, stale-content alerts. |
...
| 9b | UX-ORCH-5 | 🟡 spec done | Frontend health monitoring spec at `workspace/content-orchestrator/docs/specs/frontend-health-monitoring-spec.md`. Implementation pending (provider choice: self-hosted JSONL recommended). |
| 9c | UX-ORCH-7 | 🟡 spec done | Experiment registry spec at `workspace/content-orchestrator/docs/specs/experiment-registry-spec.md`. No active experiments; framework ready for post-freeze use. |
...
| 10a | ORCH-6 | 🟡 plan + integrity done | DR plan at `workspace/content-orchestrator/docs/specs/disaster-recovery-plan.md`. Backup script hardened with fail-closed verification, checksum logging, and size anomaly detection. Off-server backup and restore drill require owner action. |
...
| 10e | SEO-ORCH-7 | 🟡 spec done | Off-page/entity/AEO ledger spec at `workspace/content-orchestrator/docs/specs/off-page-entity-ledger-spec.md`. All outreach approval-first. |
| 10f | UX-ORCH-9 | 🟡 spec done | Customer feedback taxonomy spec at `workspace/content-orchestrator/docs/specs/customer-feedback-taxonomy-spec.md`. Start with GA4 search no-result + 404 tracking. |
...
### SEO — Striking-Distance Command (2026-06-24) — ✅ fully closed, archived to `TASKS_ARCHIVE.md`
...
| SEO-ORCH-7 | Medium | Add governed off-page/entity/AEO operations: backlink and mention inventory, outreach/GBP/social-profile/review workflow with ownership and proof, toxic/spam review, AI citation/referral monitoring, and conversion attribution. Keep all external publishing/outreach approval-first. | 🟡 spec done 2026-06-26: `workspace/content-orchestrator/docs/specs/off-page-entity-ledger-spec.md`; outreach/entity execution remains owner-gated. |
...
### Claude Code Harness Gaps (2026-06-26) — ✅ fully closed, archived to `TASKS_ARCHIVE.md`
