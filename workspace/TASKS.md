# Emart Task Board
Last updated: 2026-06-28 (creative/video quality gates fixed; all-screen-safe reel layout sample generated)
...
| VID-9 | **Creative/video quality gates fixed.** Report: `workspace/docs/audits/creative-video-quality-audit-20260628.md`. Implemented shared `quality_gates.py`, product-type-aware daily producer templates, enqueue/worker/master-QA content gates, script placeholder/CJK/claim validation, required voice QA, caption authority handling, provider docs/config alignment, secure SSL fetch context, and HyperFrames premium render preset. Invalid COSRX SPF-copy and BOJ placeholder/CJK drafts were moved to rejected; corrected COSRX moisturizer FB/IG jobs are queued. | [X] | ✅ 2026-06-28 |
| VID-10 | **All-screen-safe reel layout.** Samsung A13/Facebook screenshots showed Meta UI covering bottom footer text. Creative Engine vertical frames now keep important product/price/domain/footer content in a central safe band and renderer QA fails 1080x1920 frames that place important elements in the lower mobile overlay zone. Sample public assets: `/public/videos/reels/sample-samsung-a13-boj-sunscreen-20260628.*`. | [X] | ✅ 2026-06-28 |
...
| 10 | 🟡 Recovery/ops maturity | ORCH-6, ORCH-7, SEO-ORCH-3, SEO-ORCH-6, SEO-ORCH-7, UX-ORCH-9, SEO-GAP-4 | Scripts verified 2026-06-27: `seo_technical_control_loop.py`, `seo_url_policy_registry.py`, `qdrant_parity_report.py`, registry JSON all exist. QDR parity 100.0% (1 orphan). SEO-ORCH-6 all 8 rules verified. Remaining: CWV/CrUX, rotating schedule, registry→middleware wiring. | Mostly post-critical; keep outreach/personal-data workflows approval-first. |
...
| 10d | SEO-ORCH-6 | ✅ all 8 rules verified 2026-06-27 | All 8 URL-policy registry rules live-verified (filter/sort/tracking params confirmed 301→clean). Remaining: middleware/metadata/robots/sitemap consumption. |
...
| D6 | Med | **GMC 83 disapproved** — healthcare claims (15), identity/belief shade names (25), personal hardships (38), illegal drugs (2), other (3). Fix 1-by-1 by sales potential | [C]+[O] | 🟡 2026-06-28: ranked 46 active IDs by sales×price; **33 fixed** — medical/hardship/identity-belief trigger language removed from `post_content`/`post_excerpt`/`_rank_math_description` via regex pass (rollback: `gmc-d6-rollback-20260628.json`); cache revalidated + GMC full sync (3,594 synced, 0 errors). **13 remain:** 8 title-level shade-name flags (foundation shades — title change needed), 3 broad-category flags (may need GMC appeal: 75273/3822/74275/63933), 1 backorder availability_date (4013, owner/stock), 1 stale image flag (58823). |
...
| SEO-ORCH-6 | Medium | Establish one versioned URL-policy registry consumed/tested by middleware, metadata, robots, sitemap, redirects and audit tools. Decide crawl/index/canonical behavior for every supported query parameter and detect drift; Googlebot currently bypasses generic robots query disallows while representative filter URLs return `index, follow` with a clean canonical. | ✅ all 8 rules live-verified 2026-06-27: `workspace/seo/url-policy-registry.json` version `2026-06-27.seo-orch-6.all-8-rules-verified`; filter/sort/tracking params confirmed 301→clean via curl. Remaining: wire registry checks into middleware/metadata/robots/sitemap tests. |
