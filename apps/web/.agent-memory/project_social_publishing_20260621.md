...
- `workspace/scripts/active/social_image_gen.py`, `social_publish.py`, and `social_publish.README.md` exist locally but are ignored by `.gitignore` (`workspace/scripts/active/*`), so future agents should verify before assuming they are committed source.
