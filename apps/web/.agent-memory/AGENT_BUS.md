...
|---|---|---|---|
| Codex | 2026-06-25 | Closed X8c Social Engine approval-status semantics by verification: unit test confirms approved campaigns set `publish_allowed`, approved/review plan dry-runs show the right publish gate, and `meta_schedule.js --publish` fails closed on `review_required` before any Meta call. TASKS now marks X8c complete; X8a GA4 product export remains open. | local commit |
