...
DEPLOY_REV_FILE="$VPS/.deployed-rev"
...
DEPLOY_SHA=$(git -C "$LOCAL" rev-parse HEAD)
DEPLOY_SHORT=$(git -C "$LOCAL" rev-parse --short HEAD)
BUILD_ID=$(cat "$VPS/$APP/.next/BUILD_ID" 2>/dev/null || echo "unknown")
{
  echo "sha=$DEPLOY_SHA"
  echo "short=$DEPLOY_SHORT"
  echo "branch=$(git -C "$LOCAL" rev-parse --abbrev-ref HEAD)"
  echo "build_id=$BUILD_ID"
  echo "deployed_at_utc=$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
  echo "source=$LOCAL"
  echo "runtime=$VPS"
} > "$DEPLOY_REV_FILE"
success "Recorded deployed revision: $DEPLOY_SHORT"
...
echo -e "  Deployed rev: $DEPLOY_REV_FILE"
